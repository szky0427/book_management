...
class BookServiceTest{
    ...
    fun createBook_正常系_書籍を登録できる(){
        ...
        `when`(bookRepository.insertBook(bookRequestDto.title, bookRequestDto.price,
            bookRequestDto.publishStatus)).thenReturn(1)

        bookService.createBook(bookRequestDto)

        verify(bookRepository, times(1)).insertBook(bookRequestDto.title, bookRequestDto.price, bookRequestDto.publishStatus)
    ...
    fun findBooksByAuthorId_正常系_著者に紐づく書籍を取得(){
        val authorId = 1
        val bookResponseDto = listOf(BookResponseDto(1, "世界の歩き方", 1000, PublishStatus.UnPublished.code, PublishStatus.UnPublished.value,
            listOf(AuthorResponseDto(1, "山田太郎", LocalDate.of(1990, 12, 25)),AuthorResponseDto(2, "佐藤花子",LocalDate.of(2000, 12, 25)))),
            BookResponseDto(2,"家庭の医学", 3000, PublishStatus.Published.code, PublishStatus.Published.value,
